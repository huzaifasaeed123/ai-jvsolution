  ...
  return (
    <>
      ...
      {googleEnabled && (
        <>
          <div className="mt-8">
            {/* Same button as sign-in: OAuth creates the account when there is
                none, so a separate "sign up with Google" would be a lie. */}
            <GoogleButton />
          </div>
          <AuthDivider />
        </>
      ...
      <form onSubmit={onSubmit} className={googleEnabled ? 'space-y-5' : 'mt-8 space-y-5'}>
        {error && (
          <p
            role="alert"
...
export function RegisterPanel({ googleEnabled }: { googleEnabled: boolean }) {
  return (
